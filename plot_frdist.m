...
function plot_frdist(sim_name, ex_c, pulse_amps, stim_amps, t, num_group, num_affected, ...
    ...
    for j = 1:length(stim_amps)
        ...
        for ex_c = analyze_coherences
            for trial = start_trial:end_trial
                ...
                stim_frs(j, :) = reshape(stim_frs(j, :), size(g1_taskfrs)) + g1_taskfrs ./ num_wins;
            end
        end
    end
    ...
    ctrl_frs = reshape(stim_frs(3, 1:top_N), [top_N, 1]);
    ...
    scatter(ball_rs*1e6, ctrl_frs, [], "k", 'filled')
