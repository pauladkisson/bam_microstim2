...
function plot_fr_heatmap(sim_name, pulse_amps, stim_amps, t, t_cut, ex_c, ...
    ...
    ctrl_cv_start = mean(ctrl_cv(beginning_of_task)); 
    an_cv_start = mean(an_cv(beginning_of_task));
    ps_cv_end = mean(ps_cv(end_of_task));
    gs_cv_end = mean(gs_cv(end_of_task));
    ctrl_cv_end = mean(ctrl_cv(end_of_task)); 
    an_cv_end = mean(an_cv(end_of_task));
